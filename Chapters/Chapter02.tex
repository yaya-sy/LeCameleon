...
\subsection{Uniform Weights Compression}
...
\subsection{Top Layers First Compression}
...
\subsection{Bottom Layers First Compression}
...
Table~\ref{tab:comparisons} shows that our approach outperforms SliceGPT \citep{ashkboos_slicegpt_2024}, a one-shot compression method for LLMs. Our approach also compares favorably with SVD-LLM \citep{svdllm} in Section \ref{ssec:svdllm}.
...
\subsection{Compatibility with quantization:}
...
\subsection{Compromise between specificity and generality:}
...
\subsection{If the weights are full rank, why still compress them?}
Our main approach to distill activations into compressed weights is based on the observation that activations are low-rank while the weights are full-rank. However, we still compress the weights and not the activations. The fact that activations are low-rank should be a motivation to compress them. Later we will discuss activation compression methods such as Matryoshka Representation Learning (MRL) and activation sparsity.

\subsection{Can an LLM be pre-trained from scratch with low-rank weights?}
